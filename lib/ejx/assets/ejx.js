...
// keep track of sub -states for cleanup?
export function stateOf(state, renderFn, to, appendMode, promises, awaiter) {
	if (!renderFn) { renderFn = () => { state.valueOf() } }
  const current = append(renderFn());
	
	state.addListener(() => {
		newdom = renderFn();
		replaceejx(current, newdom);
		current = newdom;
	});
}

export function append(items, to, appendMode, promises, awaiter) {
